...
const FilterPanel = ({ selectedRole, onFilterChange }) => {
  ...
  // Add at the top, after other tenant-specific state
  const [moveProbabilityBands, setMoveProbabilityBands] = useState({
    'Urgent (>85)': false,
    'Hot (60–85)': false,
    'Warm (<60)': false,
  });
  ...
  const renderTenantFilters = () => (
    <>
      ...
      <StyledAccordion defaultExpanded>
        ...
        <AccordionDetails sx={{ pt: 0 }}>
          <FormGroup>
            {Object.entries(moveProbabilityBands).map(([band, checked]) => (
              <FormControlLabel
                key={band}
                control={
                  <StyledCheckbox
                    checked={checked}
                    onChange={() => setMoveProbabilityBands(prev => ({ ...prev, [band]: !checked }))}
                    size="small"
                  />
                }
                label={band}
  ...
  React.useEffect(() => {
    onFilterChange && onFilterChange({
      selectedLocations,
      moveProbabilityBands,
      // ...add other filters as needed
    });
  }, [selectedLocations, moveProbabilityBands /*, ...other filters */]);
